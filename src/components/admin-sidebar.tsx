...
const adminNavLinks = [
  ...
  { href: '/admin/orders', label: 'Orders', icon: ClipboardList, notificationKey: 'pendingOrders' },
  { href: '/admin/users', label: 'Users', icon: Users },
  { href: '/admin/admins', label: 'Admins', icon: Shield },
  { href: '/admin/coupons', label: 'Coupons', icon: TicketPercent },
  { href: '/admin/live-chat', label: 'Live Chat', icon: MessageSquare, notificationKey: 'unreadChats' },
    ...
    <aside className="hidden w-64 flex-col border-r bg-background p-4 md:flex">
      ...
      <nav className="flex flex-col gap-2">
        {adminNavLinks.map((link) => {
            const notifCount = link.notificationKey ? notifications[link.notificationKey as keyof typeof notifications] : 0;
            return (
              <Link
                key={link.href}
                href={link.href}
              ...
              >
                ...
                {notifCount > 0 && link.notificationKey === 'pendingOrders' && (
                  <Badge className="h-6 w-6 justify-center p-0">{notifCount}</Badge>
                )}
                {notifCount && link.notificationKey === 'unreadChats' && (
